...
namespace til // Terminal Implementation Library. Also: "Today I Learned"
{
    ...
    public:
        ...
        void operator()(const std::string_view& in, std::string_view& out) noexcept
        {
            size_t capacity = base::CheckAdd(in.length(), _partialsLen).ValueOrDie();
            ...
            if (in.empty())
            {
                ...
                return; // the partial is populated
            }
    ...
    private:
        ...
        std::basic_string<char> _buffer; // buffer to which the populated string_view refers
        std::array<char, 4> _utfPartials; // buffer for code units of a partial code point that have to be cached
    ...
    public:
        ...
        // - E_ABORT       - the resulting string length would exceed the max_size and thus, the processing was aborted
        // - E_UNEXPECTED  - an unexpected error occurred
        void operator()(const std::wstring_view& in, std::wstring_view& out) noexcept
        {
            size_t remainingLength{ in.length() };
            size_t capacity = base::CheckAdd(remainingLength, _partialsLen).ValueOrDie();
            ...
            if (in.empty())
            {
                ...
                return; // the high surrogate is populated
            }
    ...
    private:
        ...
        std::basic_string<wchar_t> _buffer; // buffer to which the populated string_view refers
        std::array<wchar_t, 4> _utfPartials; // buffer for code units of a partial code point that have to be cached
        size_t _partialsLen{}; // number of cached code units
    };
